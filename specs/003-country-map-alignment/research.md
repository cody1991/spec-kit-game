...
describe('WorldScene mapping integration', () => {
  it('should initialize commanders with correct country mappings', async () => {
    ...
    // 验证秦始皇占领的国家确实是中国（156）
    const territories = Array.from(state.territoryStates.entries())
      .filter(([_, ts]) => ts.ownerId === qinCommander.id);
...
test('秦始皇占领的国家名称与地图位置匹配', async ({ page }) => {
  ...
  const qinColor = await page.evaluate(() => {
    const commander = window.__gameStore.getState().commanders
      .find(c => c.name === '秦始皇');
